...
pnpm build      # import + ng build (prerender) -> dist/browser
pnpm build:release  # internal staged build used by the shared release command
pnpm start      # release-aware server at http://127.0.0.1:3060 (HOST/PORT env; health: /healthz)
```
...
server/index.mjs            release-aware static server (shared lib; /healthz; retained chunks; SSG directory indexes)
bin/sync-from-ghostwriter   atomic release script (guarded by a signature of the ☑ set)
...
importer. A change invokes
`node ../server-ops/bin/site-release.mjs --site wargr --apply`, which builds away from the live tree,
and the signature stamp is written only after the command succeeds, so a failed build is retried.
