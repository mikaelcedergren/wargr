...
- Presentation changes use formatting, types, a production browser build, and the affected real
  article or home route in the already-running local product on port `4260`.
- Changes to the tracked generated article snapshot additionally run the repository's build and
  snapshot contracts. They never read Ghostwriter.
- E2E changes run the isolated repository-owned E2E command.
- Dependencies, repository authority, server code, image masters, content import/transaction and
  publisher machinery, installers, service/release definitions, and this verifier's trust
  implementation use the complete `pnpm check` gate.
- Unclassified source changes fail conservatively into the complete gate.

Ordinary verification reads only Wargr's tracked presentation snapshot. Only the existing explicit
content-sync transaction may read Ghostwriter, and this verifier never invokes it. The verifier also
never starts, stops, or repairs the development environment. Receipts and screenshots stay in
